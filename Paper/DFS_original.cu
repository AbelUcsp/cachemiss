...
__host__ graph* read_data(const char* file) {
  ...
  cout << rows << " rows/n";
	...
	dataset->data = new float[nonzeros];
...
//	clock_t begin = clock();
	//const char* filename = "/content/drive/MyDrive/Paralelos/Dataset/lpi_klein3.mtx";
//	const char* filename = "/content/drive/MyDrive/Paralelos/Dataset/dwt_869.mtx";
	//const char* filename = "/content/drive/MyDrive/Paralelos/Dataset/Dubcova1.mtx";
	//const char* filename = "/content/drive/MyDrive/Paralelos/Dataset/OPF_6000.mtx";
  //const char* filename = "/content/drive/MyDrive/Paralelos/Dataset/epb3.mtx";
  const char* filename = "/content/drive/MyDrive/Paralelos/Dataset/ky2010.mtx";

	printf("Gefsdfsdsdfsdfsdfsdfsdfix!\n");

	tupla* return_value = read_data(filename);
	...
	cudaMemcpy(data_gpu, dataset_graph->dataset->data, dataset_graph->edges * sizeof(int), cudaMemcpyHostToDevice);

	cudaMalloc((void**)&row_gpu, dataset_graph->edges * sizeof(int));
	cudaMemcpy(row_gpu, dataset_graph->dataset->row, dataset_graph->edges * sizeof(int), cudaMemcpyHostToDevice);

	cudaMalloc((void**)&column_gpu, dataset_graph->edges * sizeof(int));
	cudaMemcpy(column_gpu, dataset_graph->dataset->column, dataset_graph->edges * sizeof(int), cudaMemcpyHostToDevice);

	cudaMalloc((void**)&index_column_gpu, dataset_graph->vertices * sizeof(int));
	cudaMemcpy(index_column_gpu, dataset_graph->dataset->index_column, dataset_graph->vertices * sizeof(int), cudaMemcpyHostToDevice);

	cudaMalloc((void**)&index_row_start_gpu, dataset_graph->vertices * sizeof(int));
	cudaMemcpy(index_row_start_gpu, dataset_graph->dataset->index_row_start, dataset_graph->vertices * sizeof(int), cudaMemcpyHostToDevice);

	cudaMalloc((void**)&index_row_end_gpu, dataset_graph->vertices * sizeof(int));
	cudaMemcpy(index_row_end_gpu, dataset_graph->dataset->index_row_end, dataset_graph->vertices * sizeof(int), cudaMemcpyHostToDevice);
