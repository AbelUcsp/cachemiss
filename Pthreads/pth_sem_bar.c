...
#  endif
   ...
   for (i = 0; i < BARRIER_COUNT; i++) {
      sem_wait(&count_sem);   ///bloquea si es 0 de otro modo disminuye su valor
      if (counter == thread_count - 1) {  ///ultimo hilo en ingresar a barrera
         counter = 0;                     ///restablecer contador El contador se puede reutilizar,
         sem_post(&count_sem);            ///cede desbloquea semaforo
         for (j = 0; j < thread_count-1; j++)   ///notificar alos otros hilos que pueden continuar
            sem_post(&barrier_sems[i]);
      } else {
         counter++;
         sem_post(&count_sem);
         sem_wait(&barrier_sems[i]); ///bloquea
      }
#     ifdef DEBUG
      if (my_rank == 0) {
